...
namespace MVC_SYSTEM.Controllers
{
    public class UnblockCheckrollController : Controller
    {
            ...
            string sortdir = "ASC")
        {
            ...
            List<SelectListItem> DivisionIDList = new List<SelectListItem>(); //add by wani 18.6.2021

            if (WilayahID == 0 && LadangID == 0 && DivisionID == 0) //edit by wani 18.6.2021
            {
                ...
                //add by wani 18.6.2021
                DivisionIDList = new SelectList(db.tbl_Division
                    .Where(x => x.fld_Deleted == false && x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID)
                    .OrderBy(o => o.fld_ID)
                    .Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_DivisionName }), "Value", "Text").ToList();
                DivisionIDList.Insert(0, (new SelectListItem { Text = GlobalResCorp.lblChoose, Value = "0" }));
            ...
            ViewBag.DivList = DivisionIDList; //add by wani 18.6.2021
        ...
        // add by wani 23.6.2021
        public JsonResult GetDiv(int Est)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID, DivisionID = 0; //edit by wani 18.6.2021
            int? getuserid = GetIdentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
            ...
            List<SelectListItem> result2 = new List<SelectListItem>();
            result2 = new SelectList(db.tbl_Division
                   .Where(x => x.fld_Deleted == false && x.fld_LadangID == Est && x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID)
                   .OrderBy(o => o.fld_ID)
                   .Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_DivisionName }), "Value", "Text").ToList();

            return Json(result2);
        }
        //closed add by wani 23.6.2021


        public ActionResult _UnblockCheckrollEdit(Guid id)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID, DivisionID = 0; //edit by wani 14.7.2021
            int? getuserid = GetIdentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);
