...
namespace MVC_SYSTEM.Controllers
{
    ...
    public class DataEntryController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult _PalmPriceAdd(ModelsCorporate.tbl_HargaSawitSemasa hargaSemasa, string JenisTanamanList)
        {
            //Check_Balik
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    TempData["message"] = GlobalResCorp.msgAdd;
                    if (appname != "/")
                    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult _PalmPriceEdit(ModelsCorporate.vw_HargaSemasa hargaSemasa)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    TempData["message1"] = GlobalResCorp.msgUpdate;
                    db.Dispose();
