...
namespace MVC_SYSTEM.Controllers
{
    ...
    public class ApprovalController : Controller
    {
        ...
        public ActionResult ApprovalNewWorker(string kdldg, string ascfilename)
        {
            int[] wlyhid = new int[] { };
            //string mywlyid = "";
            ...
            string sourceFile = CheckSharedFolder.GetSourceTargetPath("filesourcepathworkerapp", NegaraID, SyarikatID); //db.tbl_OptionConfig.Where(x => x.fldOptConfFlag1 == "filesourcepathworkerapp" && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault();
            ...
            filename = db.tblASCApprovalFileDetails.Where(x => x.fldLadangID == getLadangID && x.fldWilayahID == getWilayahID && x.fldSyarikatID == SyarikatID && x.fldNegaraID == NegaraID && x.fldGenStatus == 1).Select(s => s.fldFileName).ToArray();
            myfilename = String.Join("", filename); ;

            if (kdldg != null && ascfilename != null)
            {
                files = files.Where(f => f.Name == ascfilename).OrderBy(f => f.Name).ToArray();
            }
            else
            {
                files = files.Where(f => !filename.Contains(f.Name)).OrderBy(f => f.Name).ToArray();
            }
            
            IOrderedEnumerable<FileInfo> getFiles;
            if (kdldg != null && ascfilename != null)
            {
                var getFilesDB = db.tblASCApprovalFileDetails.Where(x => x.fldFileName.Trim() == ascfilename.Trim() && x.fldCodeLadang.Trim() == kdldg.Trim()).Select(s => s.fldFileName).ToArray();
                getFiles = files.Where(f => f.Extension == ".asc" && !getFilesDB.Contains(f.Name)).OrderBy(o => o.CreationTime);
            }
            else
            {
                getFiles = files.Where(f => f.Extension == ".asc" && f.LastWriteTime.Year == year).OrderBy(o => o.CreationTime);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ApprovalNewWorker(int? WilayahIDList, int LadangIDList, int YearList, string kodladang)
        {
            DirectoryInfo salesFTPDirectory = null;
            FileInfo[] files = null;
            ...
            filename = db.tblASCApprovalFileDetails.Where(x => x.fldLadangID == LadangIDList && x.fldWilayahID == WilayahIDList && x.fldSyarikatID == SyarikatID && x.fldNegaraID == NegaraID && x.fldGenStatus == 1).Select(s => s.fldFileName).ToArray();
            myfilename = String.Join("", filename); ;
            files = files.Where(f => !filename.Contains(f.Name)).OrderBy(f => f.Name).ToArray();
            var salesFiles = files.Where(f => f.Extension == ".asc" && f.LastWriteTime.Year == YearList).OrderBy(o => o.CreationTime);

            return View(salesFiles);
        }

        public ActionResult GetWorkerApprovalFile(string filename, int ladangid, int wilayahid)
        {
            LocalizationAttribute LocalizationAttribute = new LocalizationAttribute("ms");
            string linkfile = "";
            bool success = false;
            string msg = "";
            string status = "";
            ...
            string sourceFile = CheckSharedFolder.GetSourceTargetPath("filesourcepathworkerapp", NegaraID, SyarikatID); //db.tbl_OptionConfig.Where(x => x.fldOptConfFlag1 == "filesourcepathworkerapp" && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault();
            string destinationPath = Server.MapPath("~/Files/" + getuserid + "/WorkerApproval/");
            sourceFile = sourceFile + ladangcode + "\\" + filename;

            destinationPath = destinationPath + ladangcode;
            ...
            try
            {
                ...
                if (fileexisting == null)
                {
                    ...
                    tblASCApprovalFileDetail.fldFileName = filename;
                    tblASCApprovalFileDetail.fldCodeLadang = ladangcode;
                    tblASCApprovalFileDetail.fldNegaraID = NegaraID;
                    tblASCApprovalFileDetail.fldSyarikatID = SyarikatID;
                    tblASCApprovalFileDetail.fldLadangID = ladangid;
                    tblASCApprovalFileDetail.fldWilayahID = wilayahid;
                    tblASCApprovalFileDetail.fldGenStatus = 0;

                    db.tblASCApprovalFileDetails.Add(tblASCApprovalFileDetail);
                    db.SaveChanges();

                    var RawDataApproval = db.tblASCApprovalRawDatas.Where(x => x.fldUserID == getuserid).ToList();
                    if (RawDataApproval != null)
                    {
                        db.tblASCApprovalRawDatas.RemoveRange(RawDataApproval);
                        db.SaveChanges();
                    }
                    ReadASC.GetDataFromASCFile(destinationFile, getuserid, filename, NegaraID, SyarikatID, wilayahid, ladangid, tblASCApprovalFileDetail.fldID);
                    //db3.sp_AddTotblPkjmastApp(tblASCApprovalFileDetail.fldID);
                    fileid = tblASCApprovalFileDetail.fldID;
                }
                else
                {
                ...
                var checktaskremaider = db.tblTaskRemainders.Where(x => x.fldFileName == filename).Count();
                if (checktaskremaider == 0)
                {
                    var negaraid = db2.tbl_Negara.Find(NegaraID);
                    DatabaseAction.InsertDataTotbltblTaskRemainder(filename, ladangcode, negaraid.fld_NegaraID, SyarikatID, wilayahid, ladangid, "01");
                }

                linkfile = domain + "/Approval/GetWorkerApprovalDetail/" + fileid;
                success = true;
            }
            catch (Exception ex)
            {
        ...
        public ActionResult GetWorkerApprovalDetail(long id)
        {
            ViewBag.Approval = "class = active";
            ViewBag.fileid = id;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetWorkerApprovalDetail(long[] approval, int fileid, long[] duplicate, string Tolak)
        {
            ViewBag.Approval = "class = active";
            ...
            try
            {
                if (Tolak == null)
                {
                    if (approval != null)
                    {
                        ...
                        var approvalupdatenotaccept = db.tblPkjmastApp.Where(x => !approval.Contains(x.fldID) && x.fldFileID == fileid).ToList();
                        approvalupdatenotaccept.ForEach(x => x.fldStatus = 2);
                        approvalupdatenotaccept.ForEach(x => x.fldDateTimeApprove = timezone.gettimezone());
                        approvalupdateaccept.ForEach(x => x.fldActionBy = getuserid);
                        db.SaveChanges();
                        GenWorkerApprovalFile(fileid, out status, out msg2, out success, false);
                        if (status == 1)
                        {
                            ViewBag.Status = "Status telah berjaya dikemaskini, file berjaya dijana dan email telah dihantar untuk diterima ladang";
                            ViewBag.ClassStatus = "success";
                        }
                        else if (status == 2)
                        {
                            ViewBag.Status = "Status berjaya dikemaskini, file berjaya dijana dan email gagal dihantar untuk diterima ladang";
                            ViewBag.ClassStatus = "warning";
                        }
                        else if (status == 3)
                        {
                            ViewBag.Status = "Status berjaya dikemaskini, file gagal dijana dan email gagal dihantar untuk diterima ladang";
                            ViewBag.ClassStatus = "warning";
                        }
                        else if (status == 4)
                        {
                            ViewBag.Status = "Status berjaya dikemaskini, file berjaya dijana dan email telah dihantar sebelum ini untuk diterima ladang";
                            ViewBag.ClassStatus = "success";
                        }
                    }
                    else
                    {
                        ...
                        approvalupdatenotaccept.ForEach(x => x.fldStatus = 3);
                        approvalupdatenotaccept.ForEach(x => x.fldDateTimeApprove = timezone.gettimezone());
                    ...
                    if (duplicate != null)
                    {
                        var duplicatedata = db.tblPkjmastApp.Where(x => duplicate.Contains(x.fldID)).ToList();
                        duplicatedata.ForEach(x => x.fldStatus = 3);
                        duplicatedata.ForEach(x => x.fldDateTimeApprove = timezone.gettimezone());
                        db.SaveChanges();
                    }
                }
                else
                {
                    ...
                    approvalupdatenotaccept.ForEach(x => x.fldActionBy = getuserid);
                    db.SaveChanges();
                    GenWorkerApprovalFile(fileid, out status, out msg2, out success, true);
                    if (status == 1)
                    {
                        ViewBag.Status = "Status telah berjaya dikemaskini dan email telah dihantar untuk diterima ladang";
                        ViewBag.ClassStatus = "success";
                    }
                    else if (status == 2)
                    {
                        ViewBag.Status = "Status berjaya dikemaskini email gagal dihantar untuk diterima ladang";
                        ViewBag.ClassStatus = "warning";
                    }
                    else if (status == 3)
                    {
                        ViewBag.Status = "Status berjaya dikemaskini dan email gagal dihantar untuk diterima ladang";
                        ViewBag.ClassStatus = "warning";
                    }
                    else if (status == 4)
                    {
                        ViewBag.Status = "Status berjaya dikemaskini dan email telah dihantar sebelum ini untuk diterima ladang";
                        ViewBag.ClassStatus = "success";
                    }
                }
            }
        ...
        public void GenWorkerApprovalFile(long fileid, out int status, out string msg2, out bool success, bool rejectemail)
        {
            //string msg2 = "";
            //string status = "";
            //bool success = false;
            ...
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);
            CreateASC CreateASC = new CreateASC();
            try
            {
                if (rejectemail == false)
                {
                    CreateASC.CreateASCFileApprovalWorker(fileid, NegaraID, SyarikatID);

                    string subject = "Permohonan Pekerja Baru Telah Diluluskan";
                    string msg = "";
                    ...
                    msg += "<tbody>";
                    msg += "<tr>";
                    msg += "<td align=\"center\">" + getreceiverdetail.fld_NamaWilayah + "</td><td align=\"center\">" + getreceiverdetail.fld_LdgCode + "</td><td align=\"center\">" + getreceiverdetail.fld_NamaLadang + "</td><td align=\"center\">" + getfilename.fldFileName + "</td><td align=\"center\">" + ActionBy + "</td><td align=\"center\">" + getdatetime + "</td>";
                    msg += "</tr>";
                    msg += "</tbody>";
                    msg += "</table>";
                    msg += "<p>Sila buat penerimaan dengan segara.</p>";
                    msg += "<p>Terima Kasih.</p>";
                    msg += "</body>";
                    msg += "</html>";

                    cclist.Add(getreceiverdetail.fld_WlyhEmail);
                    ...
                    {
                        bool emailstatus = SendEmailNotification.SendEmail(subject, msg, getreceiverdetail.fld_LdgEmail, cc, bcc);
                        if (emailstatus)
                        {
                            msg2 = "File berjaya dijana dan email berjaya dihantar";
                            status = 1;
                            SendEmailNotification.InsertIntotblEmailNotiStatus(getreceiverdetail.fld_NegaraID, getreceiverdetail.fld_SyarikatID, getreceiverdetail.fld_WilayahID, getreceiverdetail.fld_LadangID, getfilename.fldFileName, "Email From HQ To Ladang - New Worker Approval", "HQ", 1);
                        }
                        else
                        {
                            SendEmailNotification.InsertIntotblEmailNotiStatus(getreceiverdetail.fld_NegaraID, getreceiverdetail.fld_SyarikatID, getreceiverdetail.fld_WilayahID, getreceiverdetail.fld_LadangID, getfilename.fldFileName, "Email From HQ To Ladang - New Worker Approval", "HQ", 0);
                            msg2 = "File berjaya dijana dan email gagal dihantar";
                            status = 2;
                        }
                        DatabaseAction.UpdateDataTotbltblTaskRemainder(getfilename.fldFileName, getreceiverdetail.fld_NegaraID, getreceiverdetail.fld_SyarikatID, getreceiverdetail.fld_WilayahID, getreceiverdetail.fld_LadangID, "01");
                    }
                    else
                    {
                        msg2 = "File berjaya dijana dan email sudah dihantar sebelumnya";
                        status = 4;
                    }
                ...
                {
                    string subject = "Permohonan Pekerja Baru Telah Ditolak";
                    string msg = "";
                    string[] cc = new string[] { };
                    List<string> cclist = new List<string>();
                    string[] bcc = new string[] { };
                    ...
                    msg += "<body>";
                    msg += "<p>Assalamualaikum,</p>";
                    msg += "<p>Permohonan untuk pekerja baru telah ditolak. Sila rujuk pihak wilayah untuk keterangan lanjut. Keterangan fail seperti dibawah:-</p>";
                    msg += "<table border=\"1\">";
                    msg += "<thead>";
                    msg += "<tr>";
                    msg += "<th>Nama Wilayah</th><th>Kod Ladang</th><th>Nama Ladang</th><th>Nama File</th><th>Tindakan Oleh</th><th>Waktu Tindakan</th>";
                    ...
                    {
                        msg2 = "Email sudah dihantar sebelumnya";
                        status = 4;
                    }
                    DatabaseAction.UpdateDataTotblASCApprovalFileDetail(fileid);
                    success = true;
        ...
        public ActionResult ApprovalNewUserID(string kdldg, string ascfilename)
        {
            int[] wlyhid = new int[] { };
            //string mywlyid = "";
            string[] filename = new string[] { };
            ...
            string sourceFile = CheckSharedFolder.GetSourceTargetPath("filesourcepathuseridapp", NegaraID, SyarikatID); //db.tbl_OptionConfig.Where(x => x.fldOptConfFlag1 == "filesourcepathuseridapp" && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ApprovalNewUserID(int? WilayahIDList, int LadangIDList, int YearList, string kodladang)
        {
            DirectoryInfo salesFTPDirectory = null;
            FileInfo[] files = null;
        ...
        public ActionResult GetUserIDApprovalFile(string filename, int ladangid, int wilayahid)
        {
            string linkfile = "";
            bool success = false;
            string msg = "";
            ...
            string sourceFile = CheckSharedFolder.GetSourceTargetPath("filesourcepathuseridapp", NegaraID, SyarikatID); //db.tbl_OptionConfig.Where(x => x.fldOptConfFlag1 == "filesourcepathuseridapp" && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault();
            string destinationPath = Server.MapPath("~/Files/" + getuserid + "/UserIDApproval/");
            sourceFile = sourceFile + ladangcode + "\\" + filename;
            ...
            try
            {
                ...
                if (fileexisting == null)
                {
                    ...
                    ReadASC.GetDataFromASCFile(destinationFile, getuserid, filename, NegaraID, SyarikatID, wilayahid, ladangid, tblASCApprovalFileDetail.fldID);
                    //db3.sp_AddTotblUserIDApp(tblASCApprovalFileDetail.fldID);
                    fileid = tblASCApprovalFileDetail.fldID;
                }
                else
                {
                ...
                {
                    var negaraid = db2.tbl_Negara.Find(NegaraID);
                    DatabaseAction.InsertDataTotbltblTaskRemainder(filename, ladangcode, negaraid.fld_NegaraID, SyarikatID, wilayahid, ladangid, "02");
                }

                linkfile = domain + "/Approval/GetUserIDApprovalDetail/" + fileid;
                success = true;
            }
            catch (Exception ex)
            {
        ...
        public ActionResult GetUserIDApprovalDetail(long id)
        {
            ViewBag.Approval = "class = active";
            ViewBag.fileid = id;

            var getnulldata = db4.tblUserIDApps.Where(x => x.fldNama == null).ToList();
            if (getnulldata.Count() > 0)
            {
                db4.tblUserIDApps.RemoveRange(getnulldata);
                db4.SaveChanges();
            }

            var getapprovaldetail = db4.tblUserIDApps.Where(x => x.fldFileID == id).ToList();
            ViewBag.getdata = db4.tblUserIDApps.Where(x => x.fldFileID == id && x.fldStatus == "1").Count();
            return View(getapprovaldetail);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult GetUserIDApprovalDetail(long[] approval, int fileid, long[] duplicate, string Tolak)
        {
            ViewBag.Approval = "class = active";
            ViewBag.fileid = fileid;
            int status = 0;
            ...
            try
            {
                if (Tolak == null)
                {
                    if (approval != null)
                    {
                        var approvalupdateaccept = db4.tblUserIDApps.Where(x => approval.Contains(x.fldID)).ToList();
                        approvalupdateaccept.ForEach(x => x.fldStatus = "1");
                        approvalupdateaccept.ForEach(x => x.fldDateTimeApprove = timezone.gettimezone());
                        approvalupdateaccept.ForEach(x => x.fldActionBy = getuserid);
                        db4.SaveChanges();
                        ...
                        GenUserIDApprovalFile(fileid, out status, out msg2, out success, false);
                        ...
                        {
                            ViewBag.Status = "Status berjaya dikemaskini dan file berjaya dijana dan email gagal dihantar untuk diterima ladang";
                            ViewBag.ClassStatus = "warning";
                        }
                        else if (status == 3)
                        {
                            ViewBag.Status = "Status berjaya dikemaskini dan file gagal dijana dan email gagal dihantar untuk diterima ladang";
                            ViewBag.ClassStatus = "warning";
                        }
                        else if (status == 4)
                        {
                    ...
                    {
                        var approvalupdatenotaccept = db4.tblUserIDApps.Where(x => x.fldFileID == fileid).ToList();
                        approvalupdatenotaccept.ForEach(x => x.fldStatus = "3");
                        approvalupdatenotaccept.ForEach(x => x.fldDateTimeApprove = timezone.gettimezone());
                        approvalupdatenotaccept.ForEach(x => x.fldActionBy = getuserid);
                    ...
                    if (duplicate != null)
                    {
                        var duplicatedata = db4.tblUserIDApps.Where(x => duplicate.Contains(x.fldID)).ToList();
                        duplicatedata.ForEach(x => x.fldStatus = "3");
                        db4.SaveChanges();
                    }
                }
                else
                {
                    ...
                    GenUserIDApprovalFile(fileid, out status, out msg2, out success, true);
            ...
            var getapprovaldetail = db4.tblUserIDApps.Where(x => x.fldFileID == fileid).ToList();
            ViewBag.getdata = db4.tblUserIDApps.Where(x => x.fldFileID == fileid && x.fldStatus == "1").Count();
            
            return View(getapprovaldetail);
        }

        public void GenUserIDApprovalFile(long fileid, out int status, out string msg2, out bool success, bool rejectemail)
        {
            //string msg2 = "";
            //string status = "";
            //bool success = false;
            ...
            try
            {
                if (rejectemail == false)
                {
                    CreateASC.CreateASCFileApprovalUserID(fileid, NegaraID, SyarikatID);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ApprovalNewUserIDOPMS(int? WilayahIDList, int LadangIDList, int YearList, string kodladang)
        {
            int[] wlyhid = new int[] { };
            //string mywlyid = "";
            string[] filename = new string[] { };
            ...
            if (kodladang != "")
            {
                ...
                if (ladangdetail != null)
                {
                    ...
                    LadangIDList2 = new SelectList(dbC.tbl_Ladang.Where(x => x.fld_ID == ladangdetail.fld_ID && x.fld_Deleted == false).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text", LadangIDList).ToList();
                }
                else
                {
                    nocodeladang = 1;
            ...
            if ((WilayahID == 0 && LadangID == 0 && kodladang == "") || (nocodeladang == 1))
            {
                ...
                if (WilayahIDList == 0)
                {
                    LadangIDList2 = new SelectList(dbC.tbl_Ladang.Where(x => wlyhid.Contains((int)x.fld_WlyhID) && x.fld_Deleted == false).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text", LadangIDList).ToList();
                }
                else
                {
                    LadangIDList2 = new SelectList(dbC.tbl_Ladang.Where(x => x.fld_WlyhID == WilayahIDList && x.fld_Deleted == false).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text", LadangIDList).ToList();
                }
            }
            else if ((WilayahID != 0 && LadangID == 0 && kodladang == "") || (nocodeladang == 1))
            {
            ...
            else if ((WilayahID != 0 && LadangID != 0 && kodladang == "") || (nocodeladang == 1))
            {
                ...
                wlyhid = getwilyah.GetWilayahID2(SyarikatID, WilayahID);
                WilayahIDList2 = new SelectList(dbC.tbl_Wilayah.Where(x => wlyhid.Contains(x.fld_ID)), "fld_ID", "fld_WlyhName", WilayahID).ToList();
                LadangIDList2 = new SelectList(dbC.tbl_Ladang.Where(x => wlyhid.Contains((int)x.fld_WlyhID) && x.fld_ID == LadangID && x.fld_Deleted == false).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text", LadangIDList).ToList();
            }
        ...
        public ActionResult ApprovalNewWorkerOPMS(int WilayahIDList = 0, int LadangIDList = 0)
        {
            ViewBag.Approval = "class = active";
            int[] wlyhid = new int[] { };
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            {
                getWilayahID = dbC.tbl_Wilayah.Where(x => x.fld_SyarikatID == SyarikatID).Select(s => s.fld_ID).Take(1).FirstOrDefault();
                //getLadangID = dbC.tbl_Ladang.Where(x => x.fld_WlyhID == getWilayahID).Select(s => s.fld_ID);
            }

            //else if (WilayahIDList != 0 && LadangIDList == 0)
            //{
            //    getWilayahID = dbC.tbl_Wilayah.Where(x => x.fld_SyarikatID == SyarikatID && x.fld_ID == WilayahIDList).Select(s => s.fld_ID).Take(1).FirstOrDefault();
            //    getLadangID = dbC.tbl_Ladang.Where(x => x.fld_WlyhID == getWilayahID).Select(s => s.fld_ID).Take(1).FirstOrDefault();
            //}
            ...
            if (WilayahIDList == 0 && LadangIDList == 0)
            {
                ...
                LadangIDList2 = new SelectList(dbC.tbl_Ladang.Where(x => x.fld_WlyhID == getWilayahID && x.fld_Deleted == false).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text").ToList();
            }
            //else if (WilayahIDList != 0 && LadangIDList == 0)
            //{
            //    wlyhid = getwilyah.GetWilayahID2(SyarikatID, WilayahIDList);
            //    WilayahIDList2 = new SelectList(dbC.tbl_Wilayah.Where(x => wlyhid.Contains(x.fld_ID)), "fld_ID", "fld_WlyhName").ToList();
            //    LadangIDList2 = new SelectList(dbC.tbl_Ladang.Where(x => wlyhid.Contains((int)x.fld_WlyhID) && x.fld_Deleted == false).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text").ToList();
            //}
            else if (WilayahIDList != 0 && LadangIDList != 0)
            {
                wlyhid = getwilyah.GetWilayahID(SyarikatID);
                WilayahIDList2 = new SelectList(dbC.tbl_Wilayah.Where(x => wlyhid.Contains(x.fld_ID)), "fld_ID", "fld_WlyhName").ToList();
                LadangIDList2 = new SelectList(dbC.tbl_Ladang.Where(x => x.fld_WlyhID == WilayahIDList && x.fld_Deleted == false).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text").ToList();
            }
            ...
            //if (WilayahID == 0 && LadangID == 0)
            //{
            //    var wilayahDetails = dbhq.tbl_Wilayah.Where(x => x.fld_SyarikatID == SyarikatID && x.fld_Deleted == false).OrderBy(o=>o.fld_ID).ToList();
            //    WilayahIDList2 = new SelectList(wilayahDetails.Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_WlyhName }), "Value", "Text").ToList();
            //    WilayahIDList2.Insert(0, (new SelectListItem { Text = "Semua", Value = "0" }));
            //    LadangIDList2 = new SelectList(dbhq.tbl_Ladang.Where(x => x.fld_WlyhID == wilayahDetails.Select(s=>s.fld_ID).FirstOrDefault() && x.fld_Deleted == false).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text").ToList();
            ...
            if (WilayahIDList == 0 && LadangIDList == 0)
            {
                //var resultreport = dbhq.tblASCApprovalFileDetails.Where(x => x.fldNegaraID == NegaraID && x.fldSyarikatID == SyarikatID && x.fldASCFileStatus==1 && x.fldPurpose==2).OrderBy(o => o.fldWilayahID);
        ...
        //        {
        //            string host2, catalog2, user2, pass2 = "";
        //            Connection.GetConnection(out host2, out catalog2, out user2, out pass2, app1.fldWilayahAsal,
        //                app1.fldSyarikatAsal, app1.fldNegaraAsal);
        //            MVC_SYSTEM_ModelsEstate dbAsal =
        //                MVC_SYSTEM_ModelsEstate.ConnectToSqlServer(host2, catalog2, user2, pass2);
        ...
        //                //update pkjmast
        //                Connection.GetConnection(out host, out catalog, out user, out pass, wlyhID, syrktID, ngraID);
        //                MVC_SYSTEM_ModelsEstate dbr =
        //                    MVC_SYSTEM_ModelsEstate.ConnectToSqlServer(host, catalog, user, pass);
        //                var app2 = dbr.tbl_Pkjmast.Where(x =>
        //                    x.fld_Nopkj == nopkj && x.fld_NegaraID == ngraID && x.fld_SyarikatID == syrktID &&
        //                    x.fld_WilayahID == wlyhID && x.fld_LadangID == ldgID).FirstOrDefault();
        //                if (app2 != null)
        //                {
        //                    app2.fld_Kdaktf = "1";
        //                    app2.fld_StatusAkaun = "1";
        ...
        //                //update cuti
        //                var CutiTransfer = dbAsal.tbl_CutiPeruntukan.Where(x =>
        //                    x.fld_NoPkj == app1.fldNoPkjAsal && x.fld_Tahun == Convert.ToInt16(year) &&
        //                    x.fld_NegaraID == app1.fldNegaraAsal && x.fld_SyarikatID == app1.fldSyarikatAsal &&
        //                    x.fld_WilayahID == app1.fldWilayahAsal && x.fld_LadangID == app1.fldLadangAsal &&
        //                    x.fld_Deleted == false);
        //                if (CutiTransfer.Count() != 0)
        //                {
        //                    foreach (var item in CutiTransfer)
        ...
        //                if (app2.fld_Kdrkyt == "MA")
        //                {
        //                    var CarumanTransfer = dbAsal.tbl_PkjCarumanTambahan.Where(x =>
        //                        x.fld_Nopkj == app1.fldNoPkjAsal && x.fld_NegaraID == app1.fldNegaraAsal &&
        //                        x.fld_SyarikatID == app1.fldSyarikatAsal && x.fld_WilayahID == app1.fldWilayahAsal &&
        //                        x.fld_LadangID == app1.fldLadangAsal && x.fld_Deleted == false);
        //                    if (CarumanTransfer.Count() != 0)
        //                    {
        //                        foreach (var item in CarumanTransfer)
        //                        {
        ...
        //                    var activeContributionCategoryData = dbhq.tbl_CarumanTambahan.Where(x => x.fld_NegaraID == ngraID && x.fld_SyarikatID == syrktID && x.fld_Deleted == false && x.fld_Default == true).Where(x => x.fld_Warganegara == 2 || x.fld_Warganegara == 3);
        ...
        //                    //sip
        //                    var activeContributionCategoryData = dbhq.tbl_CarumanTambahan.Where(x => x.fld_NegaraID == ngraID && x.fld_SyarikatID == syrktID && x.fld_Deleted == false && x.fld_Default == true).Where(x => x.fld_Warganegara == 1 || x.fld_Warganegara == 3);
        //                    foreach (var activeContribution in activeContributionCategoryData)
        //                    {
        //                        var activeSubContributionCategoryData = db.tbl_SubCarumanTambahan.Where(x => x.fld_NegaraID == ngraID && x.fld_SyarikatID == syrktID && x.fld_Deleted == false && x.fld_KodCaruman == activeContribution.fld_KodCaruman);
        //                        foreach (var activeSubContribution in activeSubContributionCategoryData)
        ...
        public ActionResult Rejectreason(int id)
        {
            ViewBag.Approval = "class = active";
            MVC_SYSTEM_ModelsCorporate dbhq = new MVC_SYSTEM_ModelsCorporate();
            var viewresult = dbhq.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "sbbTolak" && x.fldDeleted == false);
        ...
        public ActionResult Rejectreason()
        {
            ...
            dbhq.SaveChanges(); //ADD BY WANI 22.9.2020
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ApprovalIncrementSalaryOPMS(int? WilayahIDList, int LadangIDList, int YearList, string kodladang)
        {
            int[] wlyhid = new int[] { };
            //string mywlyid = "";
            string[] filename = new string[] { };
        ...
        public void GenIncrementSalaryApprovalFileOPMS(long fileid, out int status, out string msg2, out bool success, bool rejectemail)
        {
            ...
            try
            {
                if (rejectemail == false)
                {
                    ...
                    {
                        //if (SendEmailNotification.SendEmail(subject, msg, getreceiverdetail.fld_LdgEmail, cc, bcc))
                        if (SendEmailNotification.SendEmail2(subject, msg, to, cc, bcc))
                        {
                            SendEmailNotification.InsertIntotblEmailNotiStatus(getreceiverdetail.fld_NegaraID, getreceiverdetail.fld_SyarikatID, getreceiverdetail.fld_WilayahID, getreceiverdetail.fld_LadangID, getfilename.fldFileName, "Email From HQ To Ladang - New User ID Approval", "HQ", 1);
                            msg2 = "Email berjaya dihantar";
                            status = 1;
