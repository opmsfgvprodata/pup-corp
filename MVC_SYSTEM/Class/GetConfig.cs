...
namespace MVC_SYSTEM.Class
{
    public class GetConfig
    {
        ...
        public string GetCostCentCode(string code, int negara, int syarikat)
        {
            var ccCodecode = db.tbl_CCSAP.Where(x => x.fld_CCcode == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false)
                .Select(s => s.fld_CCcode).FirstOrDefault();
            return ccCodecode;
        }

        public string GetCostCentDesc(string code, int negara, int syarikat)
        {
            var ccDesc = db.tbl_CCSAP.Where(x => x.fld_CCcode == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false)
        ...
        //atun tmbah 28/8
        public string GetLadang(string code, int negara, int syarikat, int wilayah)
        {
            var ladangCode = db.tbl_Ladang.Where(x => x.fld_LdgCode == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_WlyhID==wilayah && x.fld_Deleted == false).Select(s => s.fld_LdgName).FirstOrDefault();
            return ladangCode;
        }

        public string GetKatAktPUP(string code, int negara, int syarikat) // kategori aktiviti - 
        {
            var KatAktPUPCode = db.tbl_KategoriAktiviti.Where(x => x.fld_KodKategori == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false).Select(s => s.fld_KodKategori).FirstOrDefault();
            return KatAktPUPCode;
        }

        public string GetKatAktPUPDesc(string code, int negara, int syarikat) // kategori aktiviti - 
        {
            var KatAktPUPDesc = db.tbl_KategoriAktiviti.Where(x => x.fld_KodKategori == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false).Select(s => s.fld_Kategori).FirstOrDefault();
            return KatAktPUPDesc;
        }

        public string GetJnsAktvtPUP(string code, int negara, int syarikat)
        {
            var JnsAktPUPCode = db.tbl_UpahAktiviti.Where(x => x.fld_KodAktvt == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false).Select(s => s.fld_KodAktvt).FirstOrDefault();
            return JnsAktPUPCode;
        }

        public string GetNetPUP(string code, int negara, int syarikat)
        {
            var GLCode = db.tbl_SAPGLPUP.Where(x => x.fld_GLCode == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false).Select(s => s.fld_GLCode).FirstOrDefault();
            return GLCode;
        }
        ...
        //atun tmbah 12/9
        public string GetJnsxtiviti(string code, int negara, int syarikat)
        {
            var JnsCode = db.tblOptionConfigsWebs.Where(x => x.fldOptConfValue == code && x.fldOptConfFlag1 == "activityLevel" && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fldDeleted == false)
                .Select(s => s.fldOptConfDesc).FirstOrDefault();
            return JnsCode;
        }

        public string GetNNCode(string code, int negara, int syarikat)
        {
            var NNCode = db.tbl_SAPPDPUP.Where(x => x.fld_NetworkNo == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false)
                .Select(s => s.fld_NetworkNo).FirstOrDefault();
            return NNCode;
        }

        public string GetCCCode(string code, int negara, int syarikat)
        {
            var CCCode = db.tbl_SAPCCPUP.Where(x => x.fld_CostCenter == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false)
                .Select(s => s.fld_CostCenter).FirstOrDefault();
            return CCCode;
        }

        public string GetSAPCode(string code, int negara, int syarikat)
        {
            var SAPCode = db.tbl_SAPPDPUP.Where(x => x.fld_ActivityCode == code && x.fld_NegaraID == negara && x.fld_SyarikatID == syarikat && x.fld_Deleted == false)
