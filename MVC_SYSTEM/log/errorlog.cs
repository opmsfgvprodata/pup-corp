...
namespace MVC_SYSTEM.log
{
    public class errorlog
    {
        ...
        public void testlog(string data1, string From)
        {
            string path = HttpContext.Current.Server.MapPath("~/JsonLog/JsonLog-" + timezone.gettimezone().ToString("dd-MM-yyyy") + ".txt");
            using (StreamWriter writer = new StreamWriter(path, true))
            {
                writer.WriteLine("Title : " + From + ", DateTime : " + timezone.gettimezone());
                writer.WriteLine("Data : " + data1 + ", DateTime : " + timezone.gettimezone());
                writer.Close();
            }
        }
    }
